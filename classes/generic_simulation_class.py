...
class jump_diffusion(simulationClass):
    def __init__(self, name, mar_env, corr=False):
    ...
    def generate_paths(self, fixed_seed=False, day_count=365):
        ...
        if self.correlated is False:
            # if not correlated, let's generate some random numbers
            sn1 = sn_random_numbers((1, M, I))
        else:
            # if correlated, then let's use the objects as provided in market env
            sn1 = self.random_numbers
        # standard normally distributed numbers for the jump component
...
class square_root_diffusion(simulationClass):
    def __init__(self, name, mar_env, corr=False):
        super(square_root_diffusion, self).__init__(name, mar_env, corr)
        # additional parameters needed for this class
    ...
    def update(self, initial_value=None, volatility=None, kappa=None, theta=None, final_date=None):
        ...
        :param initial_value:
        :param volatility:
        :param kappa: Mean Reversion Factor
        :param theta: Long-term mean of the process
        :param final_date:
        :return:
        """
        if initial_value is not None:
